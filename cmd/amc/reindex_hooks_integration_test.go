...
func TestHookRecoveryRealOwnerAPIAdvancesAfterPublication(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		raw := []byte(`{"type":"assistant","uuid":"fixture","message":{"id":"fixture","model":"known","usage":{"input_tokens":11,"output_tokens":3},"content":[{"type":"tool_use","id":"edit","name":"Edit","input":{"file_path":"app.js"}}]}}` + "\n")
	...
	if _, err = db.Exec("DELETE FROM hook_javascript_evidence"); err != nil {
		db.Close()
		t.Fatal(err)
	}
	db.Close()
	...
	for i := 0; i < 3; i++ {
		report, err := queueHookRebuilds(ctx, server.Client(), server.URL, "stable-recovery", 1)
		...
		retry, err := queueHookRebuilds(ctx, server.Client(), server.URL, "stable-recovery", 1)
		...
		evidence, err := s.JavaScriptHookEvidence(ctx, job.SessionID, "")
		if err != nil || evidence.State != "indexed-history" || !evidence.Edited {
	...
	done, err := queueHookRebuilds(ctx, server.Client(), server.URL, "stable-recovery", 1)
